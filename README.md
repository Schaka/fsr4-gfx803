...
![Pragmata in game on an RX 580 with the hybrid DLL at the balanced tier, with the OptiScaler 10 overlay open showing FSR 4.1.1r10 upscaling 1280x720 to 1920x1080 and an upscaler time of 8.90 ms](docs/pragmata-fsr4-411-rx580.jpeg)

The photo shows the `hybrid` DLL at the `balanced` tier, running in Pragmata on an RX 580 in
September 2026. That is the BC-250 fork's FSR 4.1.1 build with ten model passes handed back to AMD's
shaders, and the `fin25_pack39` set replacing them. Its 8.90 ms of upscaler time is in line with the
9.14 ms the same tier measured on an RX 570. The overlay reports 1280x720 upscaled to 1920x1080 and 8.90 ms of upscaler time. It names the
